...
export function UploadForm() {
  ...
  function onFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    setError(null);
    if (candidate && !ACCEPTED_EXTENSIONS.includes(candidate.name.split(".").pop()?.toLowerCase() ?? "")) { setFile(null); setError("TXT, Markdown, or PDF 파일만 분석할 수 있습니다."); return; }
    setFile(candidate);
  }
  ...
  return <form className="upload-form" onSubmit={onSubmit} noValidate>
    <label className="file-drop" htmlFor={inputId}><span className="file-icon" aria-hidden="true">↗</span><span className="file-prompt">{file ? file.name : "분석할 문서를 선택하거나 여기에 놓으세요"}</span><span className="file-hint">TXT · Markdown · PDF · 최대 10MB</span></label>
