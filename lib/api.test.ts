  ...
  it("gets persisted outputs with section source slot and segment IDs", async () => {
    const payload = {
      ...
      outputs: [{
        ...
        sections: [{ heading: "Decision", text: "Approve", source_slot_ids: ["slot-1"], source_segment_ids: ["segment-1"] }],
        version: 1,
        audience: "CEO",
        max_words: 120,
        render_config_hash: "config-1",
      }],
    };
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringMatching(/\/documents\/doc-1\/outputs$/),
      { method: "GET" },
    );
