...
export type Identity = { kind: string; section_count: number; segment_count: number; character_count: number; purpose: string | null };
export type Bottleneck = { kind: string; label: string; share: number; detail: string; segment_count: number };
export type DiffResponse = { document_id: string; analysis_run_id: string; output_id: string; output_type: string; output_version: number; entries: DiffEntry[]; counts: Record<Disposition, number>; bottlenecks: Bottleneck[]; identity: Identity };
