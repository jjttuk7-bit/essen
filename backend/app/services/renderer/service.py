...
class RendererService:
    ...
    def render(self, output_type: str, slots: Sequence[object], quality_labels: Sequence[object] = (), audience: str | None = None, max_words: int | None = None, outline: DocumentOutline | None = None) -> RenderedDocument:
        try:
            renderer = self._renderers[output_type]
        except KeyError as error:
            raise ValueError(f"Unsupported output type: {output_type}") from error
        output = renderer(slots, quality_labels, outline) if output_type == "clean_version" else renderer(slots)
        self.validate_sections(output.sections, slots)
        return self._limit_words(self._apply_audience(output, audience), max_words)

    def render_all(self, slots: Sequence[object], quality_labels: Sequence[object] = (), audience: str | None = None, max_words: int | None = None, outline: DocumentOutline | None = None) -> list[RenderedDocument]:
        return [self.render(output_type, slots, quality_labels, audience, max_words, outline) for output_type in self._renderers]
